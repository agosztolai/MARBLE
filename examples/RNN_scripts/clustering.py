...
def to_support_net(net, z, new_size=None, take_means=False, scaling=False):
    ...
    # else:
    #     old_size = 1
    m_means = torch.from_numpy(means[:, :rank]).t() / sqrt(new_size) #* sqrt(old_size) / sqrt(new_size)
    n_means = torch.from_numpy(means[:, rank: 2*rank]).t() / sqrt(new_size) #* sqrt(old_size) / sqrt(new_size)
    ...
    for i in range(n_components):
        ...
        for k in range(rank):
            m_init[k, i] = torch.from_numpy(X_reduced[k]) / sqrt(new_size) #* sqrt(old_size) / sqrt(new_size)
            n_init[k, i] = torch.from_numpy(X_reduced[rank + k]) / sqrt(new_size) #* sqrt(old_size) / sqrt(new_size)
        for k in range(net.input_size):
            wi_init[k, i] = torch.from_numpy(X_reduced[2 * rank + k])
        for k in range(net.output_size):
            wo_init[k, i] = torch.from_numpy(X_reduced[2 * rank + net.input_size + k]) / new_size#* old_size / new_size
