...
def main():    
    ...
    pos = [[[] for u in range(len(conditions))] for u in range(len(time_periods))]
    vel = [[[] for u in range(len(conditions))] for u in range(len(time_periods))]
    ...
    for day in days:
        ...
        for c, cond in enumerate(conditions):
            ...
            for p, period in enumerate(time_periods):
                ...
                for t in range(data.shape[0]):                
                    ...
                    trial = pca.transform(trial[:,period[0]:period[1]].T)
                    
                    # take all points except last
                    pos[p][c].append(trial[:-1,:])
                    
                    # extract vectors between coordinates
                    vel[p][c].append(get_vector_array(trial))
                
   
    # plt.quiver(pos[0][1][:,0],pos[0][1][:,1],vel[0][1][:,0],vel[0][1][:,1], angles='xy')
    
    # stack the trials within each condition
    pos = [[np.vstack(u) for u in v] for v in pos] # trials x time x channels
    vel = [[np.vstack(u) for u in v] for v in vel] # trials x time x channels
    
    
    pos = [item for sublist in pos for item in sublist]
    vel = [item for sublist in vel for item in sublist]
    ...
    with open('./outputs/combined_sliding_data.pkl', 'wb') as handle:
...
def analyse():
    ...
    with open('./outputs/combined_sliding_data.pkl', 'rb') as handle:
