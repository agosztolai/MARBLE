...
def main():    
    ...
    for day in days:
        ...
        # time periods
        time_periods = [(0,14),(0,12),(0,10),(0,8),(0,6),(0,4)]
         
      
        # create empty list of lists for each condition
        pos = [[[] for u in range(len(time_periods))] for u in range(len(conditions))]
        vel = [[[] for u in range(len(time_periods))] for u in range(len(conditions))]
        ...
        for c, cond in enumerate(conditions):
            ...
            for p, period in enumerate(time_periods):
                ...
                for t in range(data.shape[0]):                
                    ...
                    pos[c][p].append(trial[:-1,:])
                    
                    # extract vectors between coordinates
                    vel[c][p].append(get_vector_array(trial))
    ...
    with open('./outputs/all_data.pkl', 'wb') as handle:
