...
grad = AnisoConv()
print(K)
der = grad(y, K)#data.edge_index)#, (len(y), len(y)), K)

# rotate derivatives to match the ambient coordinate system
for i, R in enumerate(Rot):
    R = torch.tensor(R, dtype=torch.float64)
    der[i] = R @ der[i]
derder = grad(der,K)# data.edge_index, (len(y), len(y)), K)
for i, R in enumerate(Rot):
    R = torch.tensor(R, dtype=torch.float64)
    derder[i] = torch.einsum("ij,kj->ik", R, derder[i].view(2, 2)).view(1, 4)

f, (ax1, ax2, ax3) = plt.subplots(1, 3, sharey=True, figsize=(14, 3), subplot_kw={"aspect": 1})
